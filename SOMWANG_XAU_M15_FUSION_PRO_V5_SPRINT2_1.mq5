//+------------------------------------------------------------------+
//| SOMWANG_XAU_M15_FUSION_PRO_V5_SPRINT2_1.mq5                     |
//| Sprint 2.1 read-only dashboard panel.                            |
...
int OnInit()
{
   if(InpTrendEMA_Fast < 1 || InpTrendEMA_Slow <= InpTrendEMA_Fast ||
      ...
      InpPanelHeight < (276 + 9 * InpPanelRowGap) || InpPanelHeight > 700)
   {
      Print("SOMWANG FUSION V5 Sprint 2.1 init failed: invalid input parameters");
   ...
   if(!ok)
   {
      Print("SOMWANG FUSION V5 Sprint 2.1 init failed: indicator cache initialization error");
      return INIT_FAILED;
   }

   g_dashboard.Configure("SWV5_S21_DASH_", InpPanelCorner, InpPanelX, InpPanelY,
                         InpPanelWidth, InpPanelHeight, InpPanelRowGap);
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "SOMWANG XAU M15 FUSION PRO V5 SPRINT 2.1");
