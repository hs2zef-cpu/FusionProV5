...
//| Sprint 1.1 architecture hardening only.                          |
...
int OnInit()
{
   ...
   {
      Print("SOMWANG FUSION V5 Sprint 1.1 init failed: invalid input parameters");
   ...
   if(!ok)
   {
      Print("SOMWANG FUSION V5 Sprint 1.1 init failed: indicator cache initialization error");
      return INIT_FAILED;
   }

   g_dashboard.Configure("SWV5_DASH_", InpPanelX, InpPanelY);
   IndicatorSetString(INDICATOR_SHORTNAME, "SOMWANG XAU M15 FUSION PRO V5 SPRINT 1.1");
                ...
                const int &spread[])
{
   ...
   g_orchestrator.Evaluate(mode, engineInput, priceAction, trend, legacy, policy, decision);

   if(InpShowDashboard)
      g_dashboard.Render(engineInput, legacy, decision);
