//+------------------------------------------------------------------+
//| SOMWANG_XAU_M15_FUSION_PRO_V5_SPRINT3_2_1.mq5                   |
//| Sprint 3.2.1 history-token hardening.                           |
//| Original V4/V4.2 sources are preserved and not modified.          |
//+------------------------------------------------------------------+
#property strict
#property indicator_chart_window
#property indicator_plots 0
#property version "5.321"
...
int OnInit()
{
   ...
   {
      Print("SOMWANG FUSION V5 Sprint 3.2.1 init failed: invalid engine, panel, cooldown, or stale-tolerance input");
   ...
   {
      Print("SOMWANG FUSION V5 Sprint 3.2.1 init failed: invalid execution mode");
      return INIT_PARAMETERS_INCORRECT;
   }
   ...
   if(!ok)
   {
      Print("SOMWANG FUSION V5 Sprint 3.2.1 init failed: " + g_orchestrator.InitializationDiagnostic());
      return INIT_FAILED;
   }

   string initDiagnostic = g_orchestrator.InitializationDiagnostic();
   if(initDiagnostic != "")
      Print("SOMWANG FUSION V5 Sprint 3.2.1: " + initDiagnostic);

   g_dashboard.Configure("SWV5_S321_DASH_", InpPanelCorner, InpPanelX, InpPanelY,
   ...
   {
      Print("SOMWANG FUSION V5 Sprint 3.2.1 evidence warning: " + g_evidenceWriter.LastError());
      g_evidenceWriter.ClearError();
   }
   if(InpExportTrendRegressionCsv || InpExportMomentumRegressionCsv)
      Print("SOMWANG FUSION V5 Sprint 3.2.1 evidence folder: " + TerminalInfoString(TERMINAL_DATA_PATH) + "\\MQL5\\Files");
   IndicatorSetString(INDICATOR_SHORTNAME, "SOMWANG XAU M15 FUSION PRO V5 SPRINT 3.2.1");
                ...
                const int &spread[])
{
   ...
   if(InpExportTrendRegressionCsv && !g_evidenceWriter.WriteTrend(trendRegression, engineInput, decision))
   {
      string trendCsvError = g_evidenceWriter.LastError();
      if(trendCsvError != "") Print("SWV5 trend CSV warning: " + trendCsvError);
      g_evidenceWriter.ClearError();
   }
   if(InpExportMomentumRegressionCsv && !g_evidenceWriter.WriteMomentum(momentumRegression, engineInput, decision))
   {
