...
The reference enforces independent Genesis, Lease, Ledger, Sequence, Submission/Claim, Pending Request Set, and Checkpoint transaction domains. Genesis accepts only typed bootstrap DTOs, binds every domain to the immutable Genesis ID/manifest/namespace/fence, and validates complete readback before readiness. Every durable transition carries a complete typed DTO envelope, namespace/fence binding, and recomputed canonical payload digest. Request-set publication must be authoritatively reloaded before checkpoint publication. An uncertain committed Claim never recreates event-local `CLAIM_GRANTED_NOW` authority. Independent Hard Kill release authority is accepted only after complete canonical digest recomputation and semantic/reference binding.
...
- MQL direct positive assertion functions: **5**
- MQL direct negative assertion functions: **65**
- MQL direct negative assertions: **YES**
- MQL assertions compiled: **YES**
- MQL assertions executed: **NO**
- Python executable scenarios: **167** per run, repeated twice internally
