                                  ...
                                  SWV5S5_ValidationResult &result)
{
   ...
   if(!SWV5S5_IsCandidateVersion(record.contract_version) || record.authority_record_id=="" ||
      record.authority_generation==0 || record.record_digest=="" ||
      record.issuer_identity!=anchor.issuer_identity || record.issuer_policy_id!=anchor.issuer_policy_id ||
      anchor.trust_anchor_id=="" || record.status!=SWV5S5_TRUST_AUTHORIZED ||
      record.producer_component=="" || record.producer_instance=="" || record.producer_epoch==0 ||
      record.symbol=="" || record.timeframe<=0 || record.execution_mode<0 || record.execution_mode>2 ||
   ...
   const ulong generation,const ulong sequence,const string ingress_identity)
{
   ...
   if(epoch!=observed.producer_epoch || generation!=observed.authority_generation)
      return (generation>observed.authority_generation ? SWV5S5_INGRESS_EVALUATION_NEW : SWV5S5_INGRESS_EVALUATION_DENIED);
