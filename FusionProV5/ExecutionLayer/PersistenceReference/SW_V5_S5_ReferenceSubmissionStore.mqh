...
bool SWV5S5_ReferenceCanonicalSubmissionRecord(const SWV5S5_SubmissionAuthorityRecord &record,
                                                ...
                                                string &fence_digest)
{
   string durable,scope,fence,state,revision,claim_id,snapshot,clock_id,clock_authority,clock_sequence;
   if(!SWV5S5_DeriveDurableSubmissionAuthorityDigest(record,durable) ||
      ...
      !SWV5S5_CanonicalUInt("claim_clock_sequence",record.claim_clock_sequence,clock_sequence)) return false;
   canonical=scope+fence+state+revision+claim_id+snapshot+clock_id+clock_authority+clock_sequence;
   string f;
...
public:
                           ...
                           SWV5S5_ReferenceTransactionResult &transaction_result)
   {
      ...
      string proposed_canonical,proposed_scope,proposed_fence;
      ...
      if(!m_store.Load(SWV5S5_REF_DOMAIN_SUBMISSION,current_row) ||
         ...
         prepared.proposed_next_record.permit.permit_digest!=m_record.permit.permit_digest ||
         !SWV5S5_ReferenceCanonicalSubmissionRecord(prepared.proposed_next_record,
                                                     proposed_canonical,proposed_scope,proposed_fence) ||
         proposed_scope!=current_scope || proposed_fence!=current_fence ||
