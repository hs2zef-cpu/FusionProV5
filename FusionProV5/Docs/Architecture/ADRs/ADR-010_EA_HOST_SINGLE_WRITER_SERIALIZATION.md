...
Lease loss immediately revokes general mutation and publication rights. A committed but unclaimed Submission Permit is only an admission reservation and cannot migrate to a new owner. A successful Invocation Claim under ADR-016 marks the exact attempt potentially externally submitted and cannot be regenerated by another event, restart, or takeover. Every host-accepted change to an admission-invalidating authority advances the derived `admission_revision`; the underlying domain remains sole authority. Normal complete request-set and checkpoint writes route only through ADR-018 fenced publication. A failed or partial publication forces runtime-disable and reconciliation under ADR-015.
