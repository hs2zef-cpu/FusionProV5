...
Lease loss immediately revokes general mutation and publication rights. A committed but unclaimed Submission Permit is only an admission reservation and cannot migrate to a new owner. A successful Invocation Claim under ADR-016 marks the exact attempt potentially externally submitted and cannot be regenerated by another event, restart, or takeover. The host does not own a cross-domain admission counter. Under ADR-019 and ADR-020 it obtains immutable, versioned records from each sole authority, establishes one provisional coherent snapshot point by stable double collect, validates that exact snapshot, and claims in the same serialized event. Only successful Claim makes that point the completed operation's Policy Admission Linearization Point. Normal complete request-set and checkpoint writes route only through ADR-018 fenced publication. A failed or partial publication forces runtime-disable and reconciliation under ADR-015.
