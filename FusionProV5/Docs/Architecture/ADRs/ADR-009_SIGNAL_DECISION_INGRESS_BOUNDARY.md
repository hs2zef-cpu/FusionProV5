...
Governance note: this revised decision is part of the Sprint 5 Phase A.4 Final Policy Admission Linearization Candidate / In Review. It does not Architecture Lock Production Contract V5, authorize Phase B/runtime implementation, or modify the frozen Signal Engine.
...
Trust is a continuing authority, not a one-time ingress check. Current Producer Trust must be revalidated before request materialization or progression creates new increasing authority, before Submission Permit creation, and as an authority member of every ADR-019 Admission Snapshot. Every nonterminal ingress/request origin is revalidated at startup and takeover. Revocation or supersession before request materialization durably selects `TERMINALLY_BLOCKED_TRUST_REVOKED`; accepted evidence remains auditable and replay cannot resurrect it.

For an ADR-020 Increasing Execution Admission operation, explicit revocation/supersession is ordered against its conditional Policy Admission Linearization Point `P`. Revocation before `P` blocks admission and uses the applicable V5 cancellation, rejection, or reconciliation disposition without inventing a V5 state. Revocation after `P` during the same uninterrupted operation does not retroactively revoke that admission if Invocation Claim later completes; it blocks all later producer publication, increasing admission, permit/attempt, and retry while broker evidence remains admissible for reconciliation. Producer Trust time validity remains a separate Claim-completion condition: `claim_time` must be inside `[valid_from, valid_until)`, so equality or later at `valid_until` yields no Claim and no completed admission.
