...
Sprint 4.6 Phase C keeps schema V4. It replaces the candidate-only checkpoint and durable-event canonical representations with typed length-prefixed `SWV5-CHECKPOINT-V4-LP1` and `SWV5-DURABLE-EVENT-SET-V4-LP1` encodings before approval. Legacy delimiter event indexes fail closed. No migration is implemented or required because V4 has never been Architecture Locked, runtime-authorized, or emitted as durable production state. Once V4 is approved, changing either canonical representation requires a schema increment and migration review.
